...
/**
 * Created by zhenglibin on 2018/8/9.
 */
public class SelectSockets {
    public static void main(String args[]){
        try {
            new SelectSockets(8080).go();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    private int port = 8080;
    public SelectSockets(int portnum){
        this.port = portnum;
    }
    ...
    private ByteBuffer buffer2 = ByteBuffer.allocateDirect(1024);
    /**
    ...
    protected void readDataFromSocket(SelectionKey key) throws Exception
    {
        ...
        while ((count = socketChannel.read(buffer)) > 0)
        {
            ...
            {
                System.out.println("服务器收到："+ new String(new byte[] {buffer.get()}));
    ...
    private void response(SocketChannel channel) throws Exception
    {
        buffer2.clear();
        buffer2.put("recieved!\r\n".getBytes());
        buffer2.flip();
